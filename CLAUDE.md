...
🔴 **All three checks are REQUIRED on `main` as of 2026-09-04** — `visual-diff`, `templates-lint` and `money-lint`. ⚠️ **`money-lint-ratchet` is a FOURTH check name and is deliberately NOT required**: it is the advisory arm of the money ratchet and it exits 1 to show red when a budget sits higher than the code. Do not add it to `required_status_checks`, and do not read it as blocking. `strict: false` stays (a PR need not be up to date with `main`), `enforce_admins: false` stays — so an admin *can* bypass, and the standing rule is not to.
...
⚠️ **A capture is sanitized by the DEPLOYED api's `@cfs/core`, not by this repo's pin — and TWO things have to be current for it to be safe.** `capture-floor.json`'s `min_core` is the version a build must be at or above before `captureFixture` will sanitize with it (api-cloudrun#838); the guard refuses the capture when the deployed build is below it. What that guard cannot see is the floor itself falling behind: a new `pii: "mask"` tag lands in core, the number stays put, the deployed build clears it, and the newly-masked field is written to git verbatim. `deno task lint:capture-floor` is that second half (templates#213) — a fifth arm of `templates-lint` that walks the pii tag set at the floor and at the newest published core, and fails when the newest carries a tag the floor does not require.

⚠️ **That lint DETECTS on tags and RECOMMENDS on tags + routes, and the two bars are deliberately different (templates#251).** A tag set is all `core` can answer about itself, so staleness is a tag question. But a `pii: "mask"` tag whose fixture-PII ROUTE has not landed yet masks to the generic filler — api-cloudrun#837's class, *fake but the wrong kind of fake* — so the version it tells you to write must carry the route too. Measured: `core` `8404a6d` added the `statements` tags as `beta.364` and `fe3858b` added the route as `beta.365`; the tag-only bisect answered `.364`, which is higher than the floor, plausible, and defeats the guard. **So expect the recommended version to sit ABOVE the one that introduced the tag, and do not "tighten" it back down.**

🔴 **Raise `min_core` only to a version api-cloudrun has DEPLOYED.** A floor above the running build refuses every capture, including the ones the raise was meant to make safe — and the deployed version is not a fact any repository holds. Read it off the running revision: `curl -s https://api.chicagofilmsupplies.com/openapi.json | jq -r .info.version` names the Cloud Run revision, `gh release list -R chicago-film-supplies/api-cloudrun` maps it to a tag, and that tag's `deno.json` holds the pin. Measured 2026-09-06: prod served `beta.353`, so the raise from `.328` to `.351` was already satisfied.

⚠️ **The lint fails CLOSED on an unreachable registry, and `lint:deployed-enums` beside it fails OPEN. That is deliberate — do not make them agree.** A missed enum member 500s the publish webhook loudly and is re-runnable; a missed PII mask succeeds quietly and writes customer data into git history, which nothing recalls. `scripts/lint-capture-floor.ts`'s header carries the comparison.
...
⭐ **`templates-lint` now MECHANISES this rule — `scripts/lint-deployed-enums.ts`, run as its fourth guard.** It reads the enum out of the RUNNING service's `/openapi.json` and fails on a sidecar naming a member that deployment does not have, on all three axes rather than the two the incidents happened to hit. **The prose above was not enough and this is the evidence**: it was added in #157 and #199 broke one hour later, authored by a session that had it on screen. Run it yourself before merging a family PR — `deno task lint:deployed-enums` (add `--base=sandbox` for a dev PR; the default is prod).

⚠️ **Its host comes from the PR's BASE branch, and no other lint here reads outside the tree.** The deployed version is not a fact this repository holds — not in `deno.json`, not in the lockfile — so no amount of making a pin-resolving lint stricter can reach this class, and a green `templates-lint` was never weak evidence about it, it was *no* evidence. ⚠️ It fails **OPEN** on any unreachable oracle, printing `COULD NOT VERIFY` with a zero count: a network blip must not become an unclearable merge blocker on a required check. A run reporting `0 value(s) checked` is not a pass — read the count.
...
`deno task preview [name] [fixture-slug]` renders a template + fixture to `preview.html` with the same overlay the API performs (component styles → template styles → layout), registers every `partials/shared/**` and `partials/<name>/**` file as an includable partial, prints the rendered `filename` and the partials it registered, and inlines the footer partial below the body to confirm it parses. **It then screenshots that HTML to `preview.png` with headless Chromium and opens no window** — see the section below. `deno task preview:watch` re-renders on change — and **watches `partials/` too**, which it did not until 2026-08-26, so editing the shared footer used to trigger no re-render at all; it keeps the GUI window (`--open`) and skips the PNG, because a human iterating wants the live page and two browser launches per save is not that.
...
🔴 **A `preview` you have not LOOKED AT is not a verification, and this repo has no test suite, so looking is the only instrument there is.** The task now does the looking half for you: it writes `preview.png` beside the HTML with headless Chromium, so reading the render costs nothing extra. This paragraph used to carry the raw incantation to run by hand, and both of its assumptions were wrong by 2026-09-08 — it opened a GUI window by default and that window was **Safari**, the macOS default, which is the wrong ENGINE (Gotenberg renders with Chromium; WebKit distributes `table-layout: fixed` surplus differently and its font metrics differ, so the column widths you eyeball there are not the ones that ship). `$PREVIEW_BROWSER` existed to fix that and had to be exported by hand, so the default stayed wrong for anyone who had not read the comment.

| you want | pass |
|---|---|
| the PNG (default) | nothing |
| a GUI window too | `--open` — resolves to Brave/Chromium before the OS default |
| no PNG | `--no-screenshot` |
| a specific canvas | `--window-size=W,H` |
| a specific binary | `PREVIEW_BROWSER_BIN=/path/to/chromium` |

⚠️ **The canvas HEIGHT is measured from the document, and that is load-bearing rather than tidy.** Old-headless `--screenshot` captures the VIEWPORT, not the full page, so a fixed height silently crops the bottom off a long document — `long-multi-group` measures 3408px and any hard-coded 1100×900 would have shown a third of it while looking like a complete render. The width is pinned at **1280 to match `DEFAULT_SCREENSHOT_WIDTH_PX` in `api-cloudrun/src/lib/templates/golden.ts`**, so a local look and a `visual-diff` candidate are the same layout; that constant is private there and reaches no shared package, so this is a deliberate copy and drifting costs the agreement.
